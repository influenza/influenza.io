...
let totaldia =0
    export function TabelaPag(props){
        ...
        const tabPage= parseInt(location.pathname.split("/tabela/")[1])
      ...
      navigate("/equipevisao")
        ...
        const funcData = async () => {
            ...
            axios.get(`http://ec2-44-220-83-117.compute-1.amazonaws.com/api/mq135Reading/v1/team/${temhandle}?page=1&direction=asc`, headers)
                ...
                .then((res) => {
                    const dataList = res.data._embedded.mQ135ReadingVOList.slice(1000*(tabPage-1),1000*((tabPage-1)+1));
...
return(
    ...
    {erroSemEmi}
        ...
        <div id="mdfoottab" style={{justifyContent:"right",marginLeft:"-20px"}}>
            ...
            {arraypages.map((item,index)=>{
                 if(item+1 == tabPage){
                    return(
                        <>
                     <div onClick={()=>{navigate(`/tabela/${item}`)}} style={{backgroundColor:"blue",width:"30px",height:"30px"}}>{"<"}</div>
             ...
             if(item <4+(tabPage-1) && item+1>tabPage){
                return(
                    <div onClick={()=>{navigate(`/tabela/${item+1}`)}} style={{backgroundColor:"blue",width:"30px",height:"30px"}}>{item+1  }</div>
                )
             }
             if(arraypages.length>4+(tabPage-1) && item == 4+(tabPage-1)){
                return(
                    <div>...</div>
                )
             }
             ...
             if(arraypages.length == item){
                return(
                    <>
                                         <div onClick={()=>{navigate(`/tabela/${item+1}`)}} style={{backgroundColor:"blue",width:"30px",height:"30px"}}>{item+1}</div>
                                         <div onClick={()=>{navigate(`/tabela/${tabPage+1}`)}} style={{backgroundColor:"blue",width:"30px",height:"30px"}}>{">"}</div>
