...
import axios from "axios";
    ...
    export function TabelaPag(props){
        let url = window.location.pathname.replace("/tabela/","")
        console.log(url)
        let erroSemEmi
          ...
          useEffect(()=>{
            ...
            if(!Cookies.get().NomeEqui2){
              navigate("/equipevisao")
                ...
                const [Diadict, setDiadict] =useState(diadict)
                ...
                const funcData = async () => {
                    ...
                    axios.get(`http://ec2-44-220-83-117.compute-1.amazonaws.com/api/mq135Reading/v1/team/${Cookies.get().NomeEqui2}?page=1&direction=asc`, headers)
                        ...
                        .then((res) => {
                            const dataList = [...res.data._embedded.mQ135ReadingVOList].slice((parseInt(url)*100)-100 ,parseInt(url)*100);
                            const dataList2 = res.data._embedded.mQ135ReadingVOList
                            setData2(dataList2)
                            console.log(parseInt(url)*100)
                            console.log([...dataList].slice((parseInt(url)*100)-100 ,parseInt(url)*100))
                            ...
                            for (let x = 0; x < dataList.length; x++) {
                                ...
                                const date = dataList[x].timestamp.slice(0, 13);
        ...
        let datavalue1 =datavalue.sort((a, b) => b[0] - a[0]);
            ...
            if (Filtro == "MenorEmissao") {
                return (
                    <tr>
                        <td>{item+(parseInt(url)*100)-99}</td>
            ...
            console.log(diaarraychosen)
             if (Filtro == "MaiorEmissao") {
                ...
                return (
                    <tr>
                        <td>{(linhas.length-item)+(parseInt(url)*100)-99}</td>
        ...
        {diaarraychosen && TabelaType =="Hora" && !erroSemEmi &&
            ...
            diaarraychosen.map((item, index) =>{
                ...
                if (Filtro == "MenorEmissao") {
                    ...
                    return(
                        <tr>
                        <td>{index+(parseInt(url)*100)-99}</td>
                ...
                if (Filtro == "MaiorEmissao") {
                    ...
                    return(
                        <tr>
                        <td>{diaarraychosen.length-index-1+(parseInt(url)*100)-99}</td>
            ...
            mes2arraychosen.map((item, index) =>{
                ...
                return(
                    ...
                    <td>{mesvaluehora[index][0]/2}</td>
                            ...
                            return(
                                <tr>
                                <td>{mes2arraychosen.length-index-1+(parseInt(url)*100)-99}</td>
...
{arraypages.map((page, index)=>{
                if(page == 1){
                    return(
                     <>
                        <div  onClick={()=>{
                            navigate(`/tabela/${parseInt(url)-1}`)
                        ...
                        <div  id={`mdpagebutton${page}`} onClick={()=>{

                            navigate(`/tabela`)
                        }} style={{backgroundColor:"#BBD6F0", width:"25px", height:"25px", justifyContent:"center", display:"flex", textAlign:"center", alignItems:"center",color:"#0075E8", }}>{page}</div>
                        </>
                                        )
                }
                ...
                else if(Cookies.get().dataar.split(",").includes(`${page}`)){
                    console.log(page)
