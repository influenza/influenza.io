...
export function CodigoSenha() {
  ...
  useEffect(() => {
    setcodcert(Cookies.get().CodigoSenha)
...
function handlecodver(){
  ...
  if(email2 ==undefined || !erros.textContent =="CODIGO DE VERIFICAÇÃO OBRIGATORIO" ){
    ...
    Cookies.set("Senha", "123")

    const headers = {
        "Authorization": `Bearer ${Cookies.get('Token')}` // Acessando o cookie 'Token'
    };
    console.log(Cookies.get("Email"))
    axios.get(`http://ec2-44-220-83-117.compute-1.amazonaws.com/api/user/v1/email/${Cookies.get('Email')}`, headers).then((res)=>{
      console.log(res)
    })

  
    const data = {
        "username": Cookies.get('Email'), // Acessando o cookie 'Email'
        "password": Cookies.get('Senha') // Acessando o cookie 'Senha'
    };
    console.log(data)
    axios.post("http://ec2-44-220-83-117.compute-1.amazonaws.com/api/user/v1/resetPassword",
        data, { headers }) // Passando o objeto headers diretamente
        .then((res) => {
            console.log(res);
        })
        .catch((err) => {
            console.error(err); // Lidando com possíveis erros
        });
...
<div id='imgmargin' style={{display:"flex",flexDirection:"row", width:"100vw",height:"15vh",justifyContent:"",alignItems:"center",marginTop:"-30vh"}}>
<div id="imgs" >  
<img src={`${logo}`} className='img'alt="" />
  <img src={`${letraverde}`} className='img'   alt="" /></div>
    ...
    <div id='md2con' style={{backgroundColor:"white",marginTop:"50px",  boxShadow:"4px 4px 4px 3px rgba(0, 0, 0, 0.2)"}}>
    ...
    <span  className="mdbotao" style={{ padding:"0px 10px",border:"2px solid #aaa",fontSize:"18px", borderRadius:"20px",   height:"5vh",marginBottom:"5px", width:"2.5vw", display:"flex", justifyContent:"center", textAlign:"center", alignItems:"center"}}>{code[5]}</span>
